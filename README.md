...
이 프로젝트는 Naver 뉴스 검색 API를 통해 최신 뉴스를 수집하고, Google Gemini API(AI)를 사용해 본문을 분석/요약한 후 Supabase 데이터베이스에 적재하는 백그라운드 워커 서비스이자 서빙 API 서버입니다.
...
2. **본문 추출 및 AI 3줄 요약 (Gemini 2.0)**
   - 검색된 기사 링크를 통해 직접 뉴스 본문 HTML 데이터를 스크래핑합니다.
   - `gemini-2.0-flash` 모델을 사용하여 본문 내용을 기반으로 핵심 내용 **3줄 요약** 및 **핵심 키워드 3~5개**를 JSON 규격으로 자동 추출합니다.

3. **데이터베이스 적재 및 자동 로테이션 (Rotation)**
   - 수집 및 요약이 완료된 기사를 Supabase DB (`news_articles` 테이블)에 대량 저장(Bulk Insert)합니다.
...
가장 최근에 수집되어 적재된 뉴스 기사 및 AI 요약본 목록을 조회합니다.
    ...
    "data": [
      {
        ...
        "source": "한국경제",
        "category": "stock",
        "ai_summary": "1. 미국 금리 인하 소식에 코스피 상승.\n2. 반도체 대형주 중심 외인 매수세 유입.\n3. 단기 차익 실현 매물 주의 필요.",
        "ai_keywords": ["금리인하", "코스피", "반도체"],
